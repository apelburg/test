<!-- begin skins/tpl/sample/request.tpl -->
 <style>
            .sample_content_tables table tbody tr td:nth-of-type(1){
				border-top:1px solid #D6D6D6;
				border-left:1px solid #D6D6D6;
				border-bottom:1px solid #D6D6D6;
			}
			.sample_content_tables table thead tr:nth-of-type(1) td:nth-of-type(1){
				background:#D6D6D6;
				border-top:1px solid #D6D6D6;
				border-left:1px solid #D6D6D6;
			}
   ...
   <div id="sample_content">
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <td width="2%"></td>
                            <td class="sample_num_rows_check" width="2%"><input type="checkbox" name="" id="checkbox_all" onChange="check_all_for_request(this,'<?php echo $_GET['sample_page']."'"; if(isset($_GET['sort'])){echo ",'".$_GET['sort']."'";} ?>)"><label for="checkbox_all"></label></td>
                            <?php                    
        ...
        $stage_b='';//объявляем переменную
            ...
            FROM
              ...
              WHERE s.stage = 3 OR s.stage = 4 OR s.stage = 5 OR s.stage = 6 AND s.manager_id = ".$_SESSION['access']['user_id']."
                ORDER BY `s`.`$sort` ASC
                ";
			//echo $query;
            ...
            if(mysql_num_rows($result) > 0){
                    ...
                    if(isset($_GET['sort']) && $_GET['sort']=='client'){
                        ...
                        echo '<form name="request_samples" id="request_samples_2" action="modules/samples/function_send.php" method="post">
                            ...
                            <thead>
                                <tr>
                                	<td></td>
                                    <td width="2%"></td>
                                    <td  class="sample_num_rows_check" width="2%"><input id="check_table_'.$table.'" type="checkbox" name="" onClick="'.$check_on_for_request.';"><label for="check_table_'.$table.'"></label> </td>
                        ...
                        <table id="table_'.$table.'">
                            <thead>
                                <tr>
                                	<td><div style="background:#8ecdf6; width:5px; position:absolute; margin-left:-5px;"></div></td>
                                    <td width="2%"></td>
                                    <td  class="sample_num_rows_check" width="2%"><input type="checkbox" name="" id="check_table_'.$table.'" onClick="'.$check_on_for_request.';"><label for="check_table_'.$table.'"></label></td>
                                    <td colspan="9">'.$sort.'<input type="hidden" value="'.$hiden_sort.'"></td>
                                    <td width="12%"><img src="skins/images/img_design/reset_btn_minus.png" style="cursor:pointer;" onClick="comment_off(this)"></td>
                                </tr>
                            </thead>
                         ...
                         echo'
                                <tr>
                                	<td '.$stage_b.'><div style="background:#8ecdf6; width:5px; position:absolute; margin-left:-5px;"></div></td>
                                    <td class="sample_num_rows">'.$count.'</td>
                                    <td class="sample_num_rows_check"><input type="checkbox" name="c_'.$item['id_n'].'" id="check_'.$item['id_n'].'" onChange="'.$greate_row_driver_list.'" '.$disabled.'><label for="check_'.$item['id_n'].'"></label><input type="hidden" value="'.$item['client_id'].'" class="client_hidden"><input type="hidden" value="'.$item['company'].'" class="client_nickName_hidden"><input type="hidden" value="'.$item['supplier_id'].'" class="suplier_hidden"><input type="hidden" value="'.$item['nickName'].'" class="suplier_nickName_hidden"><input type="hidden" value="'.$item['client_addres'].'" class="client_addres_hidden"><input type="hidden" value="'.$item['supplier_addres'].'" class="supplier_addres"></td>
                                    <td width="10%">'.$in_table.'</td>
                                    <td width="9%" onclick="img_show_togle(this)" class="td_togle">'.$item['art'].'<div><img src="'.$img_catalog.$img_arr[$item['art']]['small'].'"></div></td>
                                    <td width="30%">'.$item['name'].'</td>
                                    <td width="6%" class="sample_num_rows_unit">
