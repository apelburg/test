...
При запуске калькулятора система смотрит есть ли у данного артикула совпадения между нанесениями прикрепленными напрямую и прикрепленными к "местам нанесения".<br />
 ...
 - если после проверки остаются какие-либо непроигнорированные нанесения прикрепленные напрямую, содается место нанесения "Стандартно" и они добавляются в него. В Итоге в калькуляторе в добавление к имеющимся у артикула местам нанесения выводится пункт "Стандартно" в котором указываются непроигнорированные "оригинальные" нанесения(из числа прикрепленных напрямую) более нигде не указанные для данного артикула<br />
 - если у артикула нет присвоенных мест нанесения, то  содается место нанесения "Стандартно" и все артикулы присвоенные напрямую добавляются в него и он единственный выводится в калькуляторе.<br />
 <br />
 <br />
 Отображение нанесений в карточке товара<br />
 - 1-ая строка. общая строка всех видов нанесений прикрепленных к данному артикулу (напрямую, через места нанесения) отображается для всех<br />
