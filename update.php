...
// пересоздаём таблицы
function table_recreate(){

	$query = "DROP TABLE os__clients_contact_information";
	$result = $mysqli->query($query) or die($mysqli->error);
	echo ($result)?"таблица <strong>os__clients_contact_information</strong> удалена успешно<br>":'error';

	$query = "DROP TABLE os__client_addres_tbl";
	$result = $mysqli->query($query) or die($mysqli->error);
	echo ($result)?"таблица <strong>os__clients_contact_information</strong> удалена успешно<br>":'error';
	...
	CREATE TABLE `os__clients_contact_information` (
	...
	$result = $mysqli->query($query) or die($mysqli->error);
	echo ($result)?"таблица <strong>os__clients_contact_information</strong> создана успешно<br>":'error';

	$query = "
		CREATE TABLE `os__client_addres_tbl` (
	...
	$result = $mysqli->query($query) or die($mysqli->error);
	echo ($result)?"таблица <strong>os__client_addres_tbl</strong> создана успешно<br>":'';
}
	...
	*/
	...
	foreach($arr as $k=>$v){
		if(trim($v['email'])!=""){
			if($i==0){
				$q .= ($r==1)?';':'';
				$q .= "INSERT INTO `os__clients_contact_information` VALUES ('',".$v['id'].",'CLIENTS_TBL','email','','".$v['email']."','')";$i++;$num_str++;
			}else{
		...
		if(trim($v['phone'])!=""){
			if($i==0){
				$q .= ($r==1)?';':'';
				$q .= "INSERT INTO `os__clients_contact_information` VALUES ('',".$v['id'].",'CLIENTS_TBL','phone','work','".addslashes($v['phone'])."','')";$i++;$num_str++;
			}else{
		...
		if(trim($v['web_site'])!=""){
			if($i==0){
				$q .= ($r==1)?';':'';
				$q .= "INSERT INTO `os__clients_contact_information` VALUES ('',".$v['id'].",'CLIENTS_TBL','web_site','','".addslashes($v['web_site'])."','')";$i++;$num_str++;
			}else{
...
function os__client_addres_tbl_update(){
	global $mysqli;
	//получаем данные из основной таблицы
	...
	foreach ($arr as $key => $value) {
		if($i == 0){
			$query .= "INSERT INTO `os__client_addres_tbl` VALUES ('','".$value['id']."','CLIENTS_TBL','office','','".addslashes($value['addres'])."','','','','','','',''), ('','".$value['id']."','CLIENTS_TBL','delivery','','".addslashes($value['delivery_address'])."','','','','','','','')";
			$i++;
			}else{
			$i++;
	...
	//импортируем
	$result = $mysqli->query($query) or die($mysqli->error);
	$query = "DELETE FROM `os__client_addres_tbl` WHERE `street` = ''";
	//вытираем пустые значения
	$result = $mysqli->query($query) or die($mysqli->error);
	if($result){
		echo 'os__client_addres_tbl_update() - выполнен';
	}else{
		echo 'error';
	}
}
...
// пересоздаём таблицы
if(isset($_GET['steep_1'])){
	table_recreate();
}

// переписываем адреса в другую таблицу
if(isset($_GET['steep_2'])){
	os__client_addres_tbl_update();
}

// перезапись таблицы клиентов по новым рейтингам
// rebuild_rate();

if(isset($_GET['steep_3'])){
	client_contact_list_update_1();
}
if(isset($_GET['steep_4'])){
