...
/*  ------------------------------------------------------------------------------ */
   class Com_pred{
       static function save_to_tbl($id_arr,$conrtol_num){
		   ...
		   $query="SELECT*FROM `".CALCULATE_TBL."` WHERE id IN('".implode("','",$id_arr)."')";//echo $query;
	   ...
	   function open_in_tbl(){
	   
	   }
	   function open_in_blank(){
	   
	   }
	   function create_list($client_id){
	       global $mysqli;
		   // шаблон ряда таблицы
		   $tpl_name = 'skins/tpl/clients/client_folder/business_offers/table_rows.tpl';
		   $fd = fopen($tpl_name,'r');
		...
		function create_list_old_version($client_id){
			...
			if(file_exists($dir_name)){
				...
				*/
				 // шаблон ряда таблицы
				 $tpl_name = 'skins/tpl/clients/client_folder/business_offers/table_rows_old_version.tpl';
				 $fd = fopen($tpl_name,'r');
				 $rows_template = fread($fd,filesize($tpl_name));
				 fclose($fd);
...
*/
	...
	function change_file_comment($file_name,$file_comment){
		global $client_id;
	    ...
	    fclose($fd);
		header('Location:?'.addOrReplaceGetOnURL('','file_name&change_file_comment'));
	}
	...
	function delete_com_offer($file,$client_id,$id){
	    global $db;
		...
		if($id != ''){
		    $query = "DELETE FROM `".COM_PRED_LIST_OLD."` WHERE `id`='".$id."'";
		    $result = mysql_query($query,$db) or die(mysql_error());
	    }
			
		header('Location:?'.addOrReplaceGetOnURL('client_id='.$client_id,'delete_com_offer'));
	}
