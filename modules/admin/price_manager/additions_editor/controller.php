<?php 
    ...
    if(!empty($_POST['dataBufferForSavingToBase'])){
			...
			if(count($toDeleteArr)>0){
			    for($i=0;$i<count($toDeleteArr);$i++){
					 ...
					 if((int)$toDeleteArr[$i]!=0){
					      $query ="DELETE FROM `".BASE__CALCULATORS_ADDITIONS."` WHERE id = '".$toDeleteArr[$i]."'";
						  //echo $query;
		...
		foreach($data->tbl_data as $val){
			...
			if($result->num_rows>0){
		       $query2 ="UPDATE `".BASE__CALCULATORS_ADDITIONS."` SET  print_id='".$usluga_id."' , type='".$val[1]."', title='".$val[2]."', value='".$val[3]."', optional='".$val[4]."', multi='".$val[5]."', target='".$val[6]."' WHERE id = '".$val[0]."'";
			   $mysqli->query($query2)or die($mysqli->error);
			}
			else{
			  
			   $query2 ="INSERT INTO `".BASE__CALCULATORS_ADDITIONS."` VALUES('','".$usluga_id."','".$val[1]."','".$val[2]."','".$val[3]."','".$val[4]."','".$val[5]."','".$val[6]."')";
			   //echo $query2;
			   $mysqli->query($query2)or die($mysqli->error);
			}
		}
	...
	$query="SELECT*FROM `".BASE__CALCULATORS_ADDITIONS."` WHERE `print_id` = '".$usluga_id."' ORDER by  id";
	//echo $query;
	$result = $mysqli->query($query)or die($mysqli->error);
	...
	function make_table_tpl($type){
	    ...
	    $row_tpl =array('','англ','заголовок','надбавка','выборочно/поумолчанию','мульти/одиноч','применение(price/summ)','');	
	    $row[] = $tr1.$td1_hidden.implode($td_td,$row_tpl).$td2.$tr2;
		
		$row_tpl = array('','','','0','1','0','price','<span class="deleteElementBtn" onclick="deleteRowFromTable(this,\''.$type.'\');">&#215;</span>');
	    $row[] = $tr1.$td1_hidden.implode($td_td,$row_tpl).$td2.$tr2;
		return $row;
	}
	...
	if(isset($tbl_data)){
		...
		$row =array('англ','заголовок','надбавка','выборочно/поумолчанию','мульти/одиноч','применение(price/summ)','');	
		array_unshift($tbl_data[0],$tr1.$td1_hidden.''.$td2.$td1.implode($td_td,$row).$td2.$tr2);	   
